                    ...
                    if result[0]==True:
                        ...
                        if y1==y3 and y1==y4:  # two lines are overlapped
                            if (x1 >= x3 and x1 <= x4) or (x1>=x4 and x1 <= x3):
                                ...
                                c1[int(x1/self.grid_size)][int(y1/self.grid_size)] = 7.0
...
for angle in range(-45,46,1):
    mirror = (['L',350,30,10,angle],['R',110,30,10,angle])
    for i in range (len(mirror)):
        mir = mirror[i]
        myMirror = Mirror(mir[0],mir[1],mir[2],mir[3],mir[4])
        all_myMirror.append(myMirror)
        covv = myMirror.count_coverage()
        myDisplayMirror = myDisplay2.draw_mirror(myMirror)
        myDisplayRayCov = myDisplay2.draw_ray_coverage(myMirror)
c22 = c2
for x in range(MAX_COLUMN) :
    for y in range(MAX_ROW) :
        if math.isclose(T[x][y],2.0) :   
            if math.isclose(c1[x][y],2.0) :
                c22 -= 1                
cov = Txcoverage+c22
print("Transmitter Coverage Area =",Txcoverage)
print("Transmitter Coverage Area =",Txcoverage*100/((MAX_ROW*MAX_COLUMN)-obs_pixels))
print("New Coverage Area =",Txcoverage+c22)
cov_percent = cov*100/((MAX_ROW*MAX_COLUMN)-obs_pixels)
print("New Covearge Area Percent =",cov_percent)
